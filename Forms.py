...
class CreateGradeForm(Form):
    first_name = StringField('First Name', [validators.Length(min=1, max=150), validators.DataRequired()])
    last_name = StringField('Last Name', [validators.Length(min=1, max=150), validators.DataRequired()])
    topic_no = IntegerField('Topic No', [validators.NumberRange(min=1, max=10, message='1 to 10 only'), validators.DataRequired()])
    topic_title = StringField('Title', [validators.Length(min=10, max=50), validators.DataRequired()])
    percentage = DecimalField('Percentage', [validators.NumberRange(min=1, max=100, message='Key in numbers only'), validators.DataRequired()])
    grade = IntegerField('Grade', [validators.NumberRange(min=1, max=4, message='Key either 1, 2, 3, 4 only'), validators.DataRequired()])
